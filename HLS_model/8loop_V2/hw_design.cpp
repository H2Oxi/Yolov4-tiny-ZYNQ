...
void loop_1(int32_t   bias[Cho_max],int8_t *In_addr,int8_t *W_addr,int8_t *Out_addr,int16_t  M0,int32_t k_loop_max,int32_t p_loop_max,int32_t c_loop_max,int32_t out_map_height,int32_t out_map_width,int32_t out_map_cho,int32_t in_map_chi,int32_t in_map_height,int32_t in_map_width);
void DMA_1(int8_t   in_L2[Tp][Tc],int8_t weights_L1[Tk][Tc],int8_t *In_addr,int8_t *W_addr,int c,int ky,int kx,int k,int p,int32_t out_map_cho,int32_t in_map_chi,int32_t in_map_height,int32_t in_map_width);
void Loop_2(int8_t   in_L2[Tp][Tc],int8_t weights_L1[Tk][Tc],int32_t   bias[Cho_max],int32_t   out[Tp][Tk],int k,int p,int8_t *Out_addr,int8_t *In_addr,int8_t *W_addr,int16_t  M0,int32_t c_loop_max,int32_t out_map_height,int32_t out_map_width,int32_t out_map_cho,int32_t in_map_chi,int32_t in_map_height,int32_t in_map_width);
...
/*void Param_load(int32_t *Param,int16_t  *M0,int32_t *k_loop_max,int32_t *p_loop_max,int32_t *c_loop_max,int32_t *out_map_height,
    int32_t *out_map_width,int32_t *out_map_cho,int32_t *in_map_chi,int32_t *in_map_height,int32_t *in_map_width)
{
...
void Conv_deal(int8_t *In_addr,int8_t *W_addr,int32_t *B_addr,int8_t *Out_addr,int32_t *Param)
{
    ...
    loop_1(   bias,In_addr, W_addr, Out_addr,  M0, k_loop_max, p_loop_max, c_loop_max,out_map_height, out_map_width, out_map_cho, in_map_chi, in_map_height, in_map_width);
	...
	for(int c=0;c<c_loop_max;c++)
		for(int ky=0;ky<K;ky++)
			for(int kx=0;kx<K;kx++)
				{
                    DMA_1(   in_L2, weights_L1, In_addr, W_addr, c, ky, kx, k, p, out_map_cho, in_map_chi, in_map_height, in_map_width);
...
void DMA_1(int8_t   in_L2[Tp][Tc],int8_t weights_L1[Tk][Tc],int8_t *In_addr,int8_t *W_addr,int c,int ky,int kx,int k,int p,
    int32_t out_map_cho,int32_t in_map_chi,int32_t in_map_height,int32_t in_map_width)
{
	for(int pp=0;pp<Tp;pp++)
	{
		if(p*Tp+pp<Pr*Pc)
		{
			...
			for(int kk=0;kk<Tk;kk++)
				for(int cc=0;cc<Tc;cc++)
					{
						if( ((c*Tc+cc)<in_map_chi)&&(p*Tp+pp<Pr*Pc)   &&  ((h*S+ky)!=0) && ((h*S+ky)!=(in_map_height+1)) &&((w*S+kx)!=0) && ((w*S+kx)!=(in_map_width+1)) )
						{
							int in_offset=((c*Tc+cc)*in_map_height+h*S+ky-1)*in_map_width+w*S+kx-1;
							in_L2[pp][cc]	    =  In_addr[in_offset];   //In_addr[c*Tc+cc][h*S+ky][w*S+kx];
							int w_offset=(((k*Tk+kk)*in_map_chi+c*Tc+cc)*K+ky)*K+kx;
							weights_L1[kk][cc]  =  W_addr[w_offset];                  //W_addr[k*Tk+kk][c*Tc+cc][ky][kx];
						}
						else if(	((h*S+ky)==0) || ((h*S+ky)==(in_map_height+1)) || ((w*S+kx)==0) || ((w*S+kx)==(in_map_width+1))	)
						{
							in_L2[pp][cc]=0;
							int w_offset=(((k*Tk+kk)*in_map_chi+c*Tc+cc)*K+ky)*K+kx;
    ...
    int32_t out_map_height,int32_t out_map_width,int32_t out_map_cho,int32_t in_map_chi,int32_t in_map_height,int32_t in_map_width)
{
    ...
    for(int k=0;k<k_loop_max;k++)
		for(int p=0;p<p_loop_max;p++)
		{
            ...
            Loop_2( in_L2, weights_L1,   bias,   out,k, p,Out_addr,In_addr, W_addr,  M0, c_loop_max,out_map_height, out_map_width, out_map_cho, in_map_chi, in_map_height, in_map_width);
